...
from vlmparse.benchpdf2md.bench_tests.benchmark_tsts import save_tests, TableTest
...
class TableTestSpec(BaseModel):
    ...
    up: Optional[str] = Field(default="", description="Cell immediately above the target cell")
    down: Optional[str] = Field(default="", description="Cell immediately below the target cell")
    left: Optional[str] = Field(default="", description="Cell immediately to the left of the target cell")
    right: Optional[str] = Field(default="", description="Cell immediately to the right of the target cell")
    top_heading: Optional[str] = Field(default="", description="Column header (cell all the way up)")
    left_heading: Optional[str] = Field(default="", description="Row header (cell all the way left)")

class TableTestsResponse(BaseModel):
    html_table: str = Field(description="The HTML table as a string")
    tests: list[TableTestSpec] = Field(description="List of exactly 3 table tests for this page", min_length=2, max_length=6)
...
async def main(gt_folder: Path, save_folder: Path, start_idx: int = 300, end_idx: int = 400):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gt_folder", type=str, default="/mnt/projects/rag-pretraitement/data/docparser/benchmarks/select_difficult_pdf/long_table/")
    parser.add_argument("--save_folder", type=str, default="/mnt/projects/rag-pretraitement/data/docparser/benchmarks/select_difficult_pdf/tests/long_table2")
