...
class FormFieldsResponse(BaseModel):
    boxes: list[BoundingBox] = Field(description="List of bounding boxes for all form fields that should be filled by hand, in format [y_min, x_min, y_max, x_max] with coordinates normalized 0-1000")
...
async def detect_form_fields(gemini_client, image: Image.Image, pdf_name: str, page_num: int):
    """Detect form fields"""
...
async def process_page(gemini_client, openai_client, image: Image.Image, pdf_name: str, 
                       page_num: int, save_folder: Path):
...
async def main(pdf_folder: Path, save_folder: Path, start_idx: int = 0, end_idx: int = None):
    ...
    for file_idx, file in tqdm(enumerate(files[start_idx:end_idx]), 
                                total=len(files[start_idx:end_idx])):
        ...
        try:
            ...
            for page_num, image in enumerate(images):
                ...
                await process_page(
                    gemini_client, openai_client, image, file.name, page_num, save_folder
                )
