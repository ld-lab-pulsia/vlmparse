...
def get_llm_params(model_name: str, uri: str | None = None):
    ...
    if model_name in ["gpt-4o", "gpt-4o-mini", "gpt-4.1", "gpt-4.1-mini", "gpt-4.1-nano", "gpt-5", "gpt-5-mini", "gpt-5-nano"]:
        base_url = None
...
class OpenAIConverterClient(BaseConverter):
    ...
    async def async_call_inside_page(self, page: Page) -> Page:
        ...
        image = page.image
        preprompt = [{"type": "text", "text": self.config.preprompt}] if self.config.preprompt else []
        postprompt = [{"type": "text", "text": self.config.postprompt}] if self.config.postprompt else []
