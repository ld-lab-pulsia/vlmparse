...
mock_pdf.__getitem__ = MagicMock(return_value=MagicMock(render=MagicMock(return_value=MagicMock(to_pil=MagicMock(return_value=fake_image)))))
...
class TestConvertCommand:
    ...
    def test_convert_without_uri_starts_server(self, cli, file_path):
        ...
        with patch("vlmparse.registries.converter_config_registry") as mock_converter_registry:
            with patch("vlmparse.registries.docker_config_registry") as mock_docker_registry:
                ...
                cli.convert(input=[str(file_path)], model="lightonocr")
    ...
    def test_convert_with_gpus(self, cli, file_path):
        ...
        with patch("vlmparse.registries.converter_config_registry"):
            with patch("vlmparse.registries.docker_config_registry") as mock_docker_registry:
                ...
                cli.convert(input=[str(file_path)], model="lightonocr", gpus="0,1")
    ...
    def test_convert_string_input(self, cli, file_path, mock_converter_client):
...
class TestCLIConvertInDepth:
    ...
    def mock_openai_api(self):
        ...
        with patch("openai.AsyncOpenAI") as mock_client_class:
            ...
            mock_response.choices[0].message.content = "# Test Document\n\nPage content here."
    ...
    def mock_docker_server_operations(self):
        ...
        with patch("vlmparse.registries.docker_config_registry") as mock_docker_registry:
            ...
            mock_server.start.return_value = ("http://localhost:8000/v1", mock_container)
            mock_docker_config.get_server.return_value = mock_server
    ...
    def test_convert_with_real_converter_gemini(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
    ...
    def test_convert_with_real_converter_lightonocr(self, cli, file_path, mock_openai_api, mock_docker_server_operations):
        """Test convert with real LightOnOCR converter, auto-starting mocked server."""
        mock_docker_registry, mock_docker_config, mock_server = mock_docker_server_operations
    ...
    def test_convert_batch_multiple_files(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
    ...
    def test_convert_verifies_document_structure(self, cli, file_path, mock_openai_api, mock_pdf_to_images, tmp_path):
        ...
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "# Page Title\n\nPage content with text."
    ...
    def test_convert_handles_api_errors_gracefully(self, cli, file_path, mock_pdf_to_images):
    ...
    def test_convert_uses_correct_model_name(self, cli, file_path, mock_openai_api, mock_pdf_to_images, model_name):
        ...
        assert call_args[1]["model"] in [model_name, "vllm-model", "lightonai/LightOnOCR-1B-1025", "nanonets/Nanonets-OCR2-3B"]
    
    def test_convert_respects_concurrency(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
    ...
    def test_convert_with_dotsocr_model(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
    ...
    def test_convert_image_preprocessing(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
        ...
        mock_openai_api.chat.completions.create = AsyncMock(side_effect=capture_messages)
    ...
    def test_convert_with_different_dpi_settings(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
    ...
    def test_convert_with_max_image_size_limit(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
    ...
    def test_convert_with_glob_pattern_real_converter(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
    ...
    def test_convert_checks_completion_kwargs(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
    ...
    def test_convert_processes_all_pages(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
    ...
    def test_convert_with_nanonet_postprompt(self, cli, file_path, mock_openai_api, mock_pdf_to_images):
        ...
        config = converter_config_registry.get("nanonets/Nanonets-OCR2-3B", uri="http://mocked-api/v1")
