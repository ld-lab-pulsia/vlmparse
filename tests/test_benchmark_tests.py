...
from benchdocparser.bench_tests.benchmark_tsts import (
    BaselineTest,
    BasePDFTest,
    TableTest,
    TestChecked,
...
class TestBasePDFTest:
    ...
    def test_negative_max_diffs(self):
        ...
        with pytest.raises(ValidationError):
            BasePDFTest(pdf="test.pdf", page=1, id="test_id", type="baseline", max_diffs=-1)
    ...
    def test_checked_enum(self):
        ...
        test = BasePDFTest(pdf="test.pdf", page=1, id="test_id", type="baseline", checked="verified")
...
class TestTextPresenceTest:
    ...
    def test_valid_present_test(self):
        ...
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="test text")
    ...
    def test_valid_absent_test(self):
        ...
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="absent", text="test text", case_sensitive=False)
        assert test.text == "test text"
    ...
    def test_empty_text(self):
        ...
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="")
        assert test.text == ""

    def test_present_text_exact_match(self):
        """Test that PRESENT test returns True for exact match"""
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="target text")
    ...
    def test_present_text_not_found(self):
        ...
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="missing text")
    ...
    def test_present_text_with_max_diffs(self):
        ...
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="target text", max_diffs=2)
    ...
    def test_absent_text_found(self):
        ...
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="absent", text="target text")
    ...
    def test_absent_text_found_diffs(self):
        ...
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="absent", text="target text", max_diffs=2)
        result, explanation = test.run("This is some target text in a document")
        assert result is False
    ...
    def test_absent_text_not_found(self):
        ...
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="absent", text="missing text")
    ...
    def test_case_insensitive_present(self):
        ...
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="TARGET TEXT", case_sensitive=False)
        result, _ = test.run("This is some target text in a document")
        assert result is True

    def test_case_insensitive_absent(self):
        """Test that case_sensitive=False works for ABSENT test"""
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="absent", text="TARGET TEXT", case_sensitive=False)
        result, explanation = test.run("This is some target text in a document")
        assert result is False

    def test_first_n_limit(self):
        """Test that first_n parameter works correctly"""
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="beginning", first_n=20)
        result, _ = test.run("beginning of text, but not the end")
        assert result is True

        # Test that text beyond first_n isn't matched
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="end", first_n=20)
        result, _ = test.run("beginning of text, but not the end")
        assert result is False

    def test_last_n_limit(self):
        """Test that last_n parameter works correctly"""
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="end", last_n=20)
        result, _ = test.run("beginning of text, but not the end")
        assert result is True

        # Test that text beyond last_n isn't matched
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="beginning", last_n=20)
        result, _ = test.run("beginning of text, but not the end")
        assert result is False

    def test_both_first_and_last_n(self):
        """Test that combining first_n and last_n works correctly"""
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="beginning", first_n=15, last_n=10)
        result, _ = test.run("beginning of text, middle part, but not the end")
        assert result is True

        # Text only in middle shouldn't be found
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="middle", first_n=15, last_n=10)
        result, _ = test.run("beginning of text, middle part, but not the end")
        assert result is False

    def test_unicode_normalized_forms(self):
        """Test that e+accent == e_with_accent unicode chars"""
        test = TextPresenceTest(pdf="test.pdf", page=1, id="test_id", type="present", text="I like to eat at a café")
...
class TestTextOrderTest:
    ...
    def test_valid_initialization(self):
        ...
        test = TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="order", before="first text", after="second text")
    ...
    def test_invalid_test_type(self):
        """Test that invalid test type raises ValidationError"""
        with pytest.raises(ValidationError):
            TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="present", before="first text", after="second text")

    def test_empty_before(self):
        """Test that empty before text is allowed (no Pydantic validation on text length)"""
        # Note: The current implementation allows empty text - no validation constraint exists
        test = TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="order", before="", after="second text")
        assert test.before == ""

    def test_empty_after(self):
        """Test that empty after text is allowed (no Pydantic validation on text length)"""
        # Note: The current implementation allows empty text - no validation constraint exists
        test = TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="order", before="first text", after="")
        assert test.after == ""

    def test_correct_order(self):
        """Test that correct order returns True"""
        test = TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="order", before="first", after="second")
        result, _ = test.run("This has first and then second in correct order")
        assert result is True

    def test_incorrect_order(self):
        """Test that incorrect order returns False"""
        test = TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="order", before="second", after="first")
    ...
    def test_before_not_found(self):
        ...
        test = TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="order", before="missing", after="present")
    ...
    def test_after_not_found(self):
        ...
        test = TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="order", before="present", after="missing")
        result, explanation = test.run("This text has present but not the other word")
        assert result is False

    def test_max_diffs(self):
        """Test that max_diffs parameter works correctly"""
        test = TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="order", before="first", after="second", max_diffs=1)
    ...
    def test_multiple_occurrences(self):
        ...
        test = TextOrderTest(pdf="test.pdf", page=1, id="test_id", type="order", before="target", after="target")
...
class TestBaselineTest:
    ...
    def test_valid_initialization(self):
        """Test that valid initialization works"""
        test = BaselineTest(pdf="test.pdf", page=1, id="test_id", type="baseline", max_repeats=50)
    ...
    def test_repeating_content(self):
        ...
        test = BaselineTest(pdf="test.pdf", page=1, id="test_id", type="baseline", max_repeats=2)
